...
Then(
  "o botão Salvar da tela de atribuição deve conter o texto {string}",
  async function (texto: string) {
    if (!pageFixture.page) {
      throw new Error("pageFixture.page não está inicializado.F");
    }
    await expect(
      pageFixture.page.locator("#btnSetAtribuicaoEscolhida"),
    ).toContainText(texto);
  },
);

Then(
  "o botão Sair da tela de atribuição deve conter o texto {string}",
  async function (texto: string) {
    if (!pageFixture.page) {
      throw new Error("pageFixture.page não está inicializado.");
    }
